...
contract Sample721Onchain is ERC721Upgradeable, AccessControlEnumerableUpgradeable, UUPSUpgradeable, ISample721Onchain {
    ...
    function mintByMinter(address to, TokenURIParam calldata tokenURIParam) public onlyRole(MINTER_ROLE) {
        uint256 tokenId = tokenIds;
        ++tokenIds;
        tokenURIParams[tokenId] = tokenURIParam;
        _mint(to, tokenId);

        emit MintByMinter(_msgSender(), to, tokenId, tokenURIParam, balanceOf(to));
    }
